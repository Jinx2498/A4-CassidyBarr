...
{
    [CreateAssetMenu(fileName = "SampleGlobalStartState", menuName = "StateMachine/States/SampleGlobalStartState")]
    public class SampleGlobalStartState : State
    {
            ...
            Event<TEvent> eventArguments)
        {
                ...
                && eventArguments.ReceiverId == stateMachine.Owner.ID)
            {
                if (VerbosityDebug)
                {
                    Log.Debug($"Event {eventArguments.EventType} received by {stateMachine.Owner.name} at time: {Time.time}");
                }
