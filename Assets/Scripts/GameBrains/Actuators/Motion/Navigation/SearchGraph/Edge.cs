...
namespace GameBrains.Actuators.Motion.Navigation.SearchGraph
{
	...
	public class Edge : ExtendedMonoBehaviour
	{
...
#if UNITY_EDITOR
			if (Editor.Tools.EditorTools.PrefabMode) { return; }
#endif
		...
		{
			if (Editor.Tools.EditorTools.PrefabMode || IsLocked) { return; }
		...
		[UnityEditor.CustomEditor(typeof(Edge))] [UnityEditor.CanEditMultipleObjects]
		public class EdgeEditor : UnityEditor.Editor
		{
			...
			public override void OnInspectorGUI()
			{
				...
				Object[] filtered = UnityEditor.Selection.GetFiltered(typeof(Edge), UnityEditor.SelectionMode.TopLevel);
				...
				if (selectedEdges.Length == 1)
				{
					if (edge.Graph != null)
					{
						...
						{
							UnityEditor.Selection.activeGameObject = edge.Graph.gameObject;
						}

						if (edge.Graph.NodeCollectionObject &&
						    GUILayout.Button("Edit node collection"))
						{
							UnityEditor.Selection.activeGameObject = edge.Graph.NodeCollectionObject;
						}
						...
						{
							UnityEditor.Selection.activeGameObject = edge.Graph.EdgeCollectionObject;
						}
					}
					...
					if (!edge.IsLocked)
					{
						...
						if (!Editor.Tools.EditorTools.PrefabMode
