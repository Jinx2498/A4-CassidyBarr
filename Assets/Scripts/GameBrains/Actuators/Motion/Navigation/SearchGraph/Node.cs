...
namespace GameBrains.Actuators.Motion.Navigation.SearchGraph
{
	...
	public class Node : ExtendedMonoBehaviour
	{
		...
		[UnityEditor.CustomEditor(typeof(Node))] [UnityEditor.CanEditMultipleObjects]
		public class NodeEditor : UnityEditor.Editor
		{
			...
			public override void OnInspectorGUI()
			{
				...
				Object[] filtered = UnityEditor.Selection.GetFiltered(typeof(Node), UnityEditor.SelectionMode.TopLevel);
				...
				if (selectedNodes.Length == 1 && node.Graph != null)
				{
					...
					{
						UnityEditor.Selection.activeGameObject = node.Graph.gameObject;
					}

					if (node.Graph.NodeCollectionObject &&
					    GUILayout.Button("Edit node collection"))
					{
						UnityEditor.Selection.activeGameObject = node.Graph.NodeCollectionObject;
					}
					...
					{
						UnityEditor.Selection.activeGameObject = node.Graph.EdgeCollectionObject;
					}
				}
