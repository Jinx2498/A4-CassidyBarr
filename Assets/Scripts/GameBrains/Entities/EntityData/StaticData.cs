...
namespace GameBrains.Entities.EntityData
{
    ...
    public class StaticData : ExtendedScriptableObject
    {
	    ...
	    public static StaticData CreateInstance(Transform t)
	    {
		    StaticData staticData = CreateInstance<StaticData>();

		    Initialize(t, staticData);

		    return staticData;
	    }

	    protected static void Initialize(Transform t, StaticData staticData)
	    {
	    ...
	    public void Reset()
	    {
		    if (ownerTransform != null)
		    {
			    ...
			    Size = meshFilter != null
				    ? Vector3.Scale(localScale, meshFilter.sharedMesh.bounds.size)
				    : localScale;
		    }
        ...
        {
	        return Owner.Color == entity.Color;
        }
