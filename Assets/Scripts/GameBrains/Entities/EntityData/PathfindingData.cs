...
namespace GameBrains.Entities.EntityData
{
    ...
    public class PathfindingData : SteeringData
    {
        ...
        public new static PathfindingData CreateInstance(Transform t)
        {
            PathfindingData pathfindingData = CreateInstance<PathfindingData>();
            Initialize(t, pathfindingData);
        ...
        {
            SteeringData.Initialize(t, pathfindingData);
        }
        ...
        public void FindPathTo(EntityTypes entityTypes)
        {
            if (steering != null) { RemoveSteeringBehaviour(steering.ID); }
            EventManager.Instance.Enqueue(
                Events.PathToEntityWithTypesRequest,
                new PathToEntityWithTypesRequestEventPayload(Owner, entityTypes));
        }
        ...
        public float CostToClosestEntityWithTypes(EntityTypes entityTypes)
        {
            float closestSoFar = float.MaxValue;
            ...
            foreach (Node node in graph.NodeCollection.Nodes)
            {
                if (!NodeIsCloseToEntityOfTypes(node, entityTypes, out Entity _)) { continue; }
        ...
        public bool NodeIsCloseToEntityOfTypes(Node node, EntityTypes entityTypes, out Entity foundEntity)
        {
            ...
            int numColliders
                = Physics.OverlapSphereNonAlloc(
                    ...
                    OverlapSphereRadius,
                    hitColliders,
                    // Exclude IgnoreRaycastLayer only
                    Physics.DefaultRaycastLayers, // TODO: Should this be Default and Obstacle layers only
            ...
            for (int i = 0; i < numColliders; i++)
            {
                ...
                if (foundEntity != null)
                {
                    var hasTypes = entityTypes.IsSubsetOf(foundEntity.EntityTypes);
                    if (hasTypes)
                    {
