    ...
    public sealed class W23ATestMappedPriorityQueue : ExtendedMonoBehaviour
    {
        ...
        public override void Update()
        {
            ...
            if (testEnqueue)
            {
                ...
                Log.Debug($"Dequeue: <{pqItem.Key}, {pqItem.Value}, {pqItem.Priority}> which should be <a,w,0>.");

                mpq.ChangeValueAndPriority("b", "v", 1);

                pqItem = mpq.Dequeue();
                
                Log.Debug($"Dequeue: <{pqItem.Key}, {pqItem.Value}, {pqItem.Priority}> which should be <b,v,1>.");

                mpq.RemoveAt(0);

                pqItem = mpq.PeekItem();
                Log.Debug($"Peek: <{pqItem.Key}, {pqItem.Value}, {pqItem.Priority}> which should be <d,7,7>.");
                
                pqItem = mpq.Dequeue();
                Log.Debug($"Dequeue: <{pqItem.Key}, {pqItem.Value}, {pqItem.Priority}> which should be <d,7,7>.");
            }
        }
